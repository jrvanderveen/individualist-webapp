...
export default (state, action) => {
    switch (action.type) {
        ...
        case "SET_INGREDIENT_LINE_THROUGH_SHOPPING_LIST":
            state.shoppingList.grocerySectionIngredientsMap[action.payload[0]].forEach((ingredient) => {
                if (ingredient._id === action.payload[1]) {
                    ingredient.lineThrough = action.payload[2];
                }
            });
            return {
                ...state,
